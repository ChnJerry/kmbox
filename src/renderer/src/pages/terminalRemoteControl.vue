<template>
  <el-table v-loading="loading" :element-loading-text="loadingText" :data="filterTableData" :max-height="450"
    ...
    <el-table-column label="ADB状态" width="110" :filters="[{ text: '未连接', value: '未连接' }, { text: '已连接', value: '已连接' }]"
      ...
      <template #default="scope">
        <el-tag :type="scope.row.adbStatus === '已连接' ? 'success' : 'info'" disable-transitions>{{
          scope.row.adbStatus ?
  ...
  <el-dialog v-model="dialogVisible" title="提示" width="30%" align-center center>
    <span style="display: flex; justify-content: center">确认重启利云终端 <text style="color: cornflowerblue">{{
      targetName
    }}</text> 吗？</span>
    <template #footer>
  ...
  <el-button style="position: absolute; bottom: 22px; right: 2px" type="info" link>{{ config.DB.tyVersion }}</el-button>
</template>

<script lang="ts" setup>
import { reactive, ref, computed, onMounted, onBeforeUnmount } from 'vue'
import { ElMessage, ElNotification, ElTag } from 'element-plus'
import { getAppConfig } from '@renderer/bridge/appConfig.js'
